...
/**
 * Logic: StateTrigger stores a trigger that triggers  its state.
 * Inside HashMap stateTransfer: We have a key stateTrigger and a trigger.
 * The trigger in hashmap is the next state we need to go.
 * */
public class StateMachine extends Component {
  ...
  private int stateIndexOf(String stateTile) {
    int index = 0;
    for (AnimationState state : states) {
      if (state.title.equals(stateTile)) {
        return index;
      }
      index++;
    }
